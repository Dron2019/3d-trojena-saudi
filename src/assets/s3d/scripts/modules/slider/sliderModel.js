...
class SliderModel extends EventEmitter {
  constructor(config, i18n) {
    ...
    this.onSliderInit = config.onSliderInit || function() {};
  }
  ...
  mouseMoveHandler(event) {
    ...
    } else if (event.target.tagName === 'polygon'  || event.target.tagName === 'g' || event.target.tagName === 'rect') {
      const config = {
...
async uploadPictures(hd) {
  ...
  const imageSrc = (index) => {
    ...
    if (self.sd_imageUrl && document.documentElement.classList.contains('tablet')) {
      return self.sd_imageUrl;
    }
  ...
  if (imageRatio > screenRatio) {
    // console.log('imge bigger ratio');
    // const onePixelChanged = this.height
    // yOffset = Math.abs((window.innerHeight - this.height) / 2);
  } else {
    // xOffset = this.width * screenRatio / -2;
    // console.log('imge smaller ratio');
  }
  ...
  uploadHdPictures() {
    if (this.hd_url) {
      ...
      axios.get(defaultModulePath+'/'+this.hd_url+this.activeElem+'.'+this.image_format, { responseType: 'blob' })
        .then(response => {
          const img = new Image();
          img.src = URL.createObjectURL(response.data);
          img.onload = () => {
            if (this.wrapperSvg) {
              this.arrayBase64Images[this.activeElem] = img;
  ...
  addToggleThemeButton() {
    ...
    const changeThemeContainerRef = document.querySelector('.s3d-ctr__theme.js-s3d-ctr__theme.s3d-ctr__menu-3d-btn-style');
    const themeChanger = themeFactory(changeThemeContainerRef);

    changeThemeRef.addEventListener('change', () => {
      themeChanger.toggle();
      const imgPath = this.theme === 'light' ? 'images/flyby/masterplanDark/' : 'images/flyby/masterplan/';
  ...
  async uploadNew(urls, progressElement, onLoadFirstKeyFrame = () => {}) {
    ...
    this.preloaderWithoutPercent.hide();
      let total = '';
      const images = [];
      const totalSize = axios.head(urls[0]).then(response => {
        total = response.headers['content-length'];
        total = +total * urls.length;
      });
      ...
      for (const url of urls) {
        ...
        if (this.controlPoint.length === 1 && index > 0) {
          images.push(images[0]);
          if (index === this.activeElem) {
            onLoadFirstKeyFrame(images[0]);
        ...
        let response = await axios.get(url+`?device=${device.type}`, { responseType: 'blob' });

        if (!response.data.type.match(/image/)) {
          retriesCount += 1;

          response = await axios.get(url+`?device=${device.type}`, { responseType: 'blob' });

          if (!response.data.type.match(/image/)) {
            window.errorPopup.setType('withoutTranslate');
            window.errorPopup.open(`Fail image`);
            throw new AppNetworkError(`Fail to load ${url}`);
          }
        }

        const urlSize = Number(response.headers['content-length']);
        loadedSize += urlSize;
  ...
  desktopZoom() {
    if (document.documentElement.classList.contains('desktop')) {
      if (this.id === 'flyby_3_outside' || this.id === 'flyby_4_outside') {
        this.desktopZoom = htmlZoom(this.wrapper, undefined, {}, 5);
      } else {
        this.desktopZoom = htmlZoom(this.wrapper);
      }
    }
  }
