...
import { closeIcon, downIcon, iconZoomLoop, infoIcon, leftIcon, minusIcon, moveIcon, plusIcon, rightIcon, upIcon } from '../assets/icons';
...
export const htmlZoom = (wrapper, scrollSelector, params = {}, maxScale = 2.5) => {
  ...
  const elementToZoom = scrollSelector ? wrapper[0].querySelector(scrollSelector) : wrapper[0].querySelector('.js-s3d__wrapper__canvas');
  const step = 30;
  ...
  // elementToZoom.style.willChange = 'transform'; 
  zoomTutorial();
  wrapper[0].addEventListener('wheel',function(evt){
